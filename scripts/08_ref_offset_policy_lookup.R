...
# ---------------------------
# 3. Explode semicolon-delimited policy fields
# ---------------------------
...
policy_lookup <- policy_exploded %>%
  mutate(
    policy_year = str_extract(policy_legal_instrument_name, "\\(?(19|20)\\d{2}\\)?") %>% 
      str_remove_all("[()]"),
    
    policy_name_clean = str_remove(policy_legal_instrument_name, "\\s*\\(?(19|20)\\d{2}\\)?$"),
...
# ---------------------------
# 5. Diagnostics: check for missing standardizations
# ---------------------------
...
if (nrow(missing_policy_names) > 0) {
  warning("⚠️ Some policy names could not be standardized:")
...
# ---------------------------
# 6. Export lookup (optional)
# ---------------------------

# write_csv(policy_lookup, here("data", "reference", "policy_lookup.csv"))

# ---------------------------
# 7. Preview (for manual review)
# ---------------------------

# View standardized policy names
print(unique(policy_lookup$policy_name_std))
