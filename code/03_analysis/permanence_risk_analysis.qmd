...
In this document, I analyze the permanence risks database I put together and cleaned in the previous scripts. The general questions I aim to address are:

1. **What permanence risks are reported in the offset literature?**
2. **How do risk patterns differ between biodiversity and carbon offsets?**
3. **What geographic, temporal, and programmatic trends exist?**
...
# Source centralized data loading script (includes all packages and datasets)
...
In this section we summarize key characteristics of the final dataset, including study counts by offset type, evidence type, geographic distribution, ecosystem focus, and programmatic context.

The dataset comprises `r total_unique_studies` unique studies spanning publications from 1990 to 2025.

### Key Dataset Statistics

The tables below show the distribution of studies across classification variables. Note that studies may appear in multiple categories (e.g., a study examining multiple countries or ecosystems).
...
Here I plot the number of studies published per year, based on the publication year of the study. 
...
## Study Characteristics by Offset Category

Below I compare the differences between biodiversity offset studies and carbon offset studies across several key variables: 
...
### Ecosystem Type x Offset Category

I extracted information from the specific ecosystem type mentioned by the authors (ecosystem_type) and then grouped that into several broad categories (ecosystem_broad_type).

In all, there were `r length(unique(final_df$ecosystem_type[!is.na(final_df$ecosystem_type)]))` specific ecosystem descriptions mentioned across the studies, which I grouped into `r length(unique(final_df$ecosystem_broad_type[!is.na(final_df$ecosystem_broad_type)]))` different ecosystem types. Like with geographic regions, some studies referenced multiple ecosystems.
...
### Project Type x Offset Category

I also tried to code the type of ecological intervention (restoration, protection, creation, etc.). I extracted what specific ecological actions the study focused on, then grouped them into 8 broad categories.
...
### Programs and Policies x Offset Category

For each study I extracted the specific offset program or policy that was the focus of the study. I then grouped these into broad categories based on the type of program or policy.
...
## Permanence Risk Patterns

This section presents the core findings on permanence risks identified across the reviewed literature. We developed a hierarchical typology with three levels: **domains** (the broadest classification), **categories** (intermediate groupings), and **types** (the most specific risks). The analysis examines risk frequencies, co-occurrence patterns, geographic variation, and temporal trends.

### Risk Typology Overview

The table below presents the complete permanence risk typology, organized by domain, category, and type. Non-physical risks (compliance, governance, financial, and socioeconomic factors) were the most frequently identified, followed by physical risks (environmental disturbances) and methodological risks (design and implementation failures).
...
ggplot(risk_long, aes(x = risk_type_label, y = n, fill = offset_category)) +
  ...
  scale_fill_manual(values = offset_colors, name = "Offset Category") +
...
#### Heatmap: Frequency of Risk Pairs x Offset Category
...
### Geographic Variation in Permanence Risks
Here I explore some of the geographic trends in permanence risks. 

#### Top Three Risks in Top Ten Most Frequent Countries
...
#### Heatmap: Risk Category Frequency by Continent
...
#### Table: Risk Category x Continent
...
### Temporal Trends in Permanence Risks

This section examines how the identification of permanence risks has evolved over time. Given the relative youth of offset programs and the lag in research publication, we might expect different risk profiles to emerge as programs mature and more empirical evidence accumulates. The dataset is skewed toward recent publications, so these trends should be interpreted cautiously.
```{r}
...
#### Top 5 Permanence Risks over Time
...
#### Temporal Trends in Risks by Program
...
Here I synthesize the major patterns I found.

### Non-Physical Risks Dominate the Literature

Non-physical risks (compliance, governance, financial, and socioeconomic factors) were the most frequently identified permanence risks across both biodiversity and carbon offset studies. These institutional and social challenges were cited more often than physical disturbances (fire, drought, flooding) or methodological failures (design flaws, metric problems).

**Key implications:**
- Offset permanence is fundamentally a governance challenge, not just an ecological one
- Physical risks like climate change impacts may be underrepresented despite their potential severity
- Investment in institutional capacity may be as important as ecological restoration

### Biodiversity and Carbon Offsets Face Different Risk Profiles

The risk patterns for biodiversity and carbon offsets differed substantially:

**Biodiversity offsets** were dominated by:
- Policy non-compliance
- Limited data transparency
- Poor management and monitoring
- Ecological design and implementation failures

**Carbon offsets** were dominated by:
- Socioeconomic and equity risks (land tenure, community exclusion, compensation gaps)
- Climate and environmental disturbances (particularly fire)
- Political and financial risks

### Geography Shapes Risk Profiles

The geographic distribution of offset research strongly influences which risks are identified:

- **Developed countries** (US, Australia, Europe): Focus on compliance, data transparency, and methodological issues, reflecting mature regulatory frameworks with detailed monitoring requirements
- **Developing countries** (tropical regions, Global South): Focus on socioeconomic and equity risks, reflecting concerns about land rights, community participation, and benefit sharing

This pattern reflects both genuine differences in risk exposure and differences in research focus and data availability.

### Program Context Matters

The two most-studied programs (US wetland mitigation and REDD+) exhibit distinct risk profiles:

- **US Wetland Mitigation**: Consistent identification of compliance and governance risks across three decades of research, despite multiple regulatory reforms (2003, 2008). This suggests institutional learning has been limited or that reforms have not addressed underlying issues.
- **REDD+**: Emergence of risks coincides with program implementation (post-2007), with strong emphasis on socioeconomic and equity considerations reflecting the program's implementation in low- and middle-income countries.

### Implications for Policy and Practice

1. **Strengthen institutional frameworks**: The predominance of non-physical risks suggests that regulatory oversight, monitoring systems, and enforcement mechanisms need continued investment.

2. **Tailor risk management to context**: Biodiversity and carbon offsets require different risk management strategies, and these should be adapted to local geographic and institutional contexts.

3. **Address equity concerns proactively**: For carbon offsets in developing countries, socioeconomic and equity risks require explicit attention in program design.

4. **Improve physical risk assessment**: Climate change will increase physical risks to offset sites; current research may underestimate these threats.

### Limitations

- The dataset is weighted toward recent publications (post-2010) and established programs (US wetland mitigation, REDD+)
- Developed countries are overrepresented, which may bias findings toward compliance-focused risks
- The review captures reported risks, which may differ from actual risk occurrence
- Risk co-occurrence patterns may reflect research approaches rather than true correlations
...
Publication-ready figures for the manuscript are generated separately in `publication_figures.qmd`. This includes high-resolution exports in PNG (600 DPI) and SVG formats with consistent color palettes.
:::
