# ================================================================
# Script:     16_clean_permanence_risk.R
# Date:       2025-06-11
# Author:     Alex Dhond
...
raw_data <- read_excel(here("data", "raw", "offset_perm_rev_database.xlsx")) %>%
  clean_names() %>%
...
risk_typology <- read_csv(here("data", "lookups", "permanence_risk_typology_lookup.csv")) %>%
...
exploded <- raw_data %>%
  select(study_id, row_id, study_title, permanence_risk_subcategory, permanence_risk_evidence_and_reasoning) %>%
...
if (nrow(unmatched_risks) > 0) {
  warning("WARNING: Some sub_risk values were not matched in the typology.")
