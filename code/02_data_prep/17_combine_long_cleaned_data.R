# ================================================================
# Script:     17_combine_long_cleaned_data.R
# Date:       2025-06-11
# Author:     Alex Dhond
...
raw_df <- read_excel(here("data", "raw", "offset_perm_rev_database.xlsx")) %>%
  clean_names() %>%
...
for (name in names(cleaned_data_list)) {
  cleaned_data_list[[name]]$df <- read_csv(here("data", "cleaned", cleaned_data_list[[name]]$file)) %>%
    mutate(across(c(study_id, study_title), as.character))
}
...
run_missing_diagnostics <- function(raw_df, cleaned_df, raw_col, cleaned_name = "cleaned", id_col = "study_title") {
  ...
  diagnostic <- full_join(raw_has_data, cleaned_has_data, by = id_col) %>%
    mutate(in_raw = replace_na(in_raw, FALSE),
           ...
           status = case_when(
             in_raw & in_cleaned  ~ "OK: Present in both",
             in_raw & !in_cleaned ~ "WARN: Missing in cleaned (check cleaning/matching)",
             !in_raw & in_cleaned ~ "WARN: Unexpected: cleaned has data, raw did not",
             TRUE                 ~ "OK: Missing in both (expected)"
           ))
...
metadata_cols <- raw_df %>%
  select(study_title, study_id, row_id, publication_year, evidence_type, offset_category_general, reviewer_notes)
...
message("Final dataset: ", nrow(final_df), " rows")
message("Unique studies: ", n_distinct(final_df$study_id))
message("Unique study_titles: ", n_distinct(final_df$study_title))
message("========================================")
